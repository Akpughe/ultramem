...
**Two conclusions reset the plan:** (1) **counting distinct events requires entity *nodes*** (each event a node), which converges with the multi‑hop/entity‑graph step — the cheap attribute‑edge counter was a dead end; (2) **measurement precision is now the gate** — pin the number with a **3× averaged run** (or a deterministic answer model) before any further synthesis tuning. The reproducible graph KU win (60→80%) remains the durable result; 78.3% is not trustworthy over 72.5%. (The inert counter code was left uncommitted — to be redone on entity nodes.)
