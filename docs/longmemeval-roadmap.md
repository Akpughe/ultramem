...
**Knowledge‑update 60% → 80% is the headline** — the four flips (5K personal best, yoga frequency, cocktail‑class day, therapist frequency) are exactly the *dated‑value resolution* cases prompting could not move: the graph surfaces the resolved current value with its event date, so the model stops abstaining or picking the stale value. Because the backfill changed *only* the graph (retrieval identical to the 66.7% run), this delta is attributable to Tier‑3. The single‑session/temporal +1/+2 are part graph, part run‑to‑run noise (±2–4); multi‑session −1 is noise; preference is untouched by the graph.

### Date‑windowed counting attempt — inert; counting needs entity nodes (negative result)

A follow‑up aimed at multi‑session: date‑windowed event counting over the graph (`count_event_instances_tagged` + `parse_window`, gated to the counting path so nothing else could regress). **It fired 0/120.** Root cause: the entity‑*attribute* schema has no countable event node — e.g. weddings scatter across `wedding_venue` / `wedding_month` / `wedding_role` edges, so "how many weddings this year" has nothing coherent to count. The re‑eval read **78.3% but was noise**: single‑session‑preference swung **45% → 70% (+5) with no preference code changed**, confirming run‑to‑run noise of **±5 on volatile categories** (preference is a judge coin‑flip). A single 120‑Q run therefore cannot measure a few‑point delta or detect a small regression.

**Two conclusions reset the plan:** (1) **counting distinct events requires entity *nodes*** (each event a node), which converges with the multi‑hop/entity‑graph step — the cheap attribute‑edge counter was a dead end; (2) **measurement precision is now the gate** — pin the number with a **3× averaged run** (or a deterministic answer model) before any further synthesis tuning. The reproducible graph KU win (60→80%) remains the durable result; 78.3% is not trustworthy over 72.5%. (The inert counter code is committed but gated off and currently a no-op — to be redone on entity nodes.)
